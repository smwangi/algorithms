 ...
 * is an abstract data type that generalizes a queue, for which elements can be added to or removed
 ...
 *
 * In these subarrays, there are  unique numbers, respectively.
...
public class JavaDeque {
    ...
    static void uniqueNumbers(int[]arr,int n, int m){
        //Deque<Integer> deque = new LinkedList<>();
        for (int i = 0; i<= n-m;i++){
            for (int j = 0; j<m;j++){
                if(i+j < n){
