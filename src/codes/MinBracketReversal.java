 ...
 * A Naive Solution is to consider every bracket and recursively count number of reversals by taking two cases
 * (i) keeping the bracket as it is (ii) reversing the bracket. If we get a balanced expression,
 * we update result if number of steps followed for reaching here is smaller than the minimum so far.
 ...
 * after removing balanced part, we always end up with an expression of the form }}…}{{…{, an expression that contains 0 or more
 * number of closing brackets followed by 0 or more numbers of opening brackets.
...
public class MinBracketReversal {
    public static void main(String[] args) {
        ...
        System.out.println(countMinReversals2(expr));
    }
    ...
    static int countMinReversals(String expr) {
        ...
        int red_len = s.size();
        // Count opening brackets at the end of the stack
        int n = 0;
     ...
     * Since the expression only contains one type of brackets, the idea is to maintain two variables to keep count of left
     * bracket as well as right bracket as we did in Length of the longest valid substring.
    ...
    static int countMinReversals2(String expr) {
        int len = expr.length();
        int ans;
        // Expression of odd lengths cannot be balanced
        if (len % 2 != 0) {
            return -1;
        }
